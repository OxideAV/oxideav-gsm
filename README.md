...
| Half rate (GSM 06.20) | **Decode chain complete (functional) + encoder analysis groundwork** — the clause 4.2 pipeline + clause 5 homing protocol; homing legs **sample-exact** against the staged GSM 06.07 corpus; waveform agreement vs `SEQ01..04.OUT` pinned in CI (mean per-frame correlation 0.42); bit-exactness is out of reach without the deliberately-unstaged GSM 06.06 fixed-point ANSI-C (clean-room posture) | The `hr` module family: annex A table A.1 parameter set (`HrParameters`, MODE-dependent subframe split), annex B `b1..b112` bit packing (tables B.1/B.2), the GSM 06.07 conformance word layouts, `hr::tables` — every ROM table of the codec transcribed from the staged data extracts — and `hr::decode` (`HrDecoder`): LPC dequantisation through the three-segment reflection-coefficient VQ, R0 energy decode, soft interpolation with the clause 4.1.6 stability fallback, VSELP + adaptive-codebook excitation with 1/6-sample fractional lags, GSP0 gain reconstruction (eqs. (131)/(132)/(145)/(146)), adaptive pitch prefilter, synthesis filter, adaptive spectral postfilter (A(z)/A(z·0,75) + brightness + AGC), and decoder homing. EN 300 969 prints the decoder as floating-point functional equations (clause 5.1 places the bit-exact arithmetic in GSM 06.06), so the chain is implemented in double precision over the staged ROM data; empirically-resolved coding ambiguities are documented in `hr::decode`. The encoder's frame-parameter analysis chain (clauses 4.1.1–4.1.6) is also in place — `hr::encode` (`HrAnalyzer`): input high-pass, FLAT covariance lattice, the three-segment AFLAT VQ search, R0 coding and the INT_LPC decision — with measured agreement against the SEQ01..03 references pinned in CI (R0 100% within one code / 95.6% exact; LPC codes 76–81% exact; the remaining disagreements are sub-LSB quantizer ties the unstaged fixed-point C resolves differently). The per-subframe excitation analysis (lag search, voicing mode, VSELP code search, gain VQ) is the next arc. |
