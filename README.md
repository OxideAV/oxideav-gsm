...
| Half rate (GSM 06.20) | **Foundation begun** — normative frame layer + complete ROM data; decode chain is the next arc | The `hr` module family: annex A table A.1 parameter set (`HrParameters`, MODE-dependent subframe split), annex B `b1..b112` bit packing for both the unvoiced (table B.1) and voiced (table B.2) layouts, the GSM 06.07 conformance word layouts (18-param + flag words, `.COD`/`.DEC` framing), and `hr::tables` — every ROM table of the codec (reflection-coefficient VQ/prequantizer/scalar dequantizer, R0 decode, table-3 allowable lags, interpolating filters, GSP0 gain VQ, sqrt(P0), VSELP basis vectors, high-pass/SST/noise-weighting/soft-interpolation coefficients) transcribed from the staged data extracts. The staged GSM 06.07 corpus (disks 1–2, `tests/fixtures/etsi-hr/`) structurally validates the layer: every real frame parses, fits its annex-A widths, and round-trips the annex-B packing. |
