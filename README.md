...
|-----------|----------|-------|
| Decoder   | §5.3 pipeline + §4.4 homing protocol (incl. §4.4 NOTE 2 / §6.3.3.2 partial detection) — **ETSI-conformant sample-exact**: the official §6 corpus (SEQ01..SEQ05 + all `*h` homing variants + HOMING01) is staged in-repo and every decoder leg reproduces the reference `*.OUT` bytes exactly in CI; additionally black-box validated over 480 000+ samples against an independent decoder binary | Fixed-point pipeline (frame unpack, LAR decode, LAR interpolation, APCM inverse, RPE grid positioning, long-term + short-term lattice synthesis, de-emphasis, §5.3.7 output shaping) plus §4.4 decoder-homing-frame detection and substitution — both the full-frame check and the §4.4 NOTE 2 / §6.3.3.2 delay-optimised partial detection (LARs + first sub-frame only, valid from the home state). The §6.2 verification configurations now run end-to-end against the §6.3.3.1 SEQ06H homing vectors (the only §6 vectors fully defined in the staged PDF) through the public registry adapters, plus a §6.3.3.2 `HOMING01`-shaped state-machine harness (`tests/conformance_homing01.rs`) that reconstructs the spec's described mixture — two complete decoder-homing-frames then interleaved complete + fractional homing frames and speech — and pins the §4.4 / §4.4-NOTE-2 reset machine frame-by-frame (non-home homes only on complete — decoding it normally and resetting *after* it; homed homes on fractional with the substituted 160 × `0x0008` output; post-reset output is history-independent). The §6.3.2 Table 6.5/6.6/6.7 spec-named boundary statements are pinned in full: the Table 6.5 mutation points (§5.2.7 `A[]`/`MAC[]`/`MIC[]`, §5.2.11 LTP-gain `mult`/`DLB[]`/opt-scaling range, §5.2.16 `FAC[]`, §5.2.4/§5.2.5/§5.2.9.2 ranges + comparisons), the Table 6.6 `smax == 0` / `L_ACF[0] == 0` paths, and all four Table 6.7 decoder-overflow `add` points (§5.3.2 / §5.3.4 / §5.3.5 / §5.3.6 saturation). The §6.3.2 SEQ05 out-of-range-`Nr` robustness ("the delay value Nr … takes in this sequence its value in [0,127]") is pinned: every `Nc ∈ [0,127]` is swept through §5.3.2 and the spec's "check the limits of Nr" rule verified (in-range used verbatim, out-of-range reuses the previous valid lag, never an OOB delay-line index). The §6.1 Table 6.1 **word-oriented conformance I/O format** is also implemented (`confio`): the right-justified 76-word `*.COD` frame (152 bytes) and the left-justified `*.INP`/`*.OUT` PCM frames (320 bytes), little- and big-endian, with a cross-format equivalence proof against the §1.7 in-band stream — this is the reader the staged corpus runs through. The bulk SEQ01..SEQ05 *binary* test sequences are now staged (`tests/fixtures/etsi-fr/`) and run bit-exactly in CI (`tests/conformance_etsi_fr.rs`) — every decoder leg (SEQ01..05, SEQ01H..05H, HOMING01, SEQ06H) reproduces the reference `*.OUT` sample-exactly. HOMING01 additionally fixed the §4.4 output-vs-reset order for a homing frame reaching a non-homed decoder (decode normally, then reset — see CHANGELOG). |
| Encoder   | §5.2 pipeline complete (§5.2.0..§5.2.18 + §1.7 packer) + §4.3 homing + §6.3.3.3 bit-sync + frame-sync — **ETSI-conformant bit-exact**: the official §6 corpus encoder legs (SEQ01..SEQ04 + `*h`, SYNC sweep) reproduce the reference `*.COD` byte-for-byte in CI; additionally black-box validated over 6 200+ frames against an independent encoder binary | Full fixed-point encode path: pre-processing, LPC analysis (autocorrelation → Schur → LAR quantisation), short-term analysis lattice, per-sub-segment LTP analysis + long-term filter, weighting filter, RPE grid selection, APCM forward quantisation, the §5.2.16..§5.2.18 local-decoder feedback loop, and the §1.7 Table 1.1 frame packer. `make_encoder` returns a working `oxideav_core::Encoder` (mono S16 8 kHz in, 33-byte frames out) with §4.3 encoder homing applied. §6.3.3.3 encoder-framing synchronization is complete in **both** steps: **bit-synchronization** (`find_bit_sync` / `run_bit_sync_trial`) and **frame-synchronization** (`FrameSyncTable` / `find_frame_sync` / `retard_special_frame`) — the 160-position special-frame sweep that recovers the encoder's 20 ms retardation, reproducing the §6.3.3.3 construction and pinning the spec's "all 160 output frames different" invariant + the full retardation round-trip (parameterised on the caller's special frame; with the corpus staged, the genuine `SEQSYNC.INP` special frame drives it). The §6 binary conformance sequences are now staged and CI-enforced: SEQ01..SEQ04 (+ `*h` variants) encode byte-exactly to the reference `*.COD`, and the genuine `SEQSYNC.INP` special frame reproduces `SYNC000.COD..SYNC149.COD` bit-exactly (the staged `SYNC15x` files are byte-duplicates of `SYNC05x` — a staged-corpus defect pinned as a tripwire, see `tests/fixtures/etsi-fr/README.md`). The checked-in black-box fixture corpus (`tests/blackbox_fixtures.rs`) additionally enforces byte-for-byte equality with an independent encoder binary. |
| Comfort noise (GSM 06.12) | §5.1 transmit evaluation + §5.2 parameter encoding + §6.1 receive generation + §6 receive-side DTX dispatch | **Transmit side** (`NoiseEvaluator`): the §5.1 `N = 4`-frame averaging of the **unquantised** parameters — per-coefficient `mean(LAR(i))` and the single `mean(xmax)` over all 16 sub-segment block maxima — then the §5.2 re-encoding *"as described in GSM 06.10"* (§5.2.7 `quantise_lar` + §5.2.15 `code_xmax`, replicated × 4) producing the `SidParameters` a SID carries. **Receive side** (`ComfortNoiseGenerator`): the §6.1 substitution (random RPE [1,6] / grid [0,3], `bcr = 0`, `Ncr = 40/120/40/120`, SID-carried LARs + block amplitudes) driving the §5.3 decoder, plus the §6.1 update-smoothing interpolation. **§6 DTX dispatch** (`DtxReceiver`): the receive-side two-state machine — speech frames decoded normally, a *valid SID* frame *"start[s] or update[s]"* comfort noise, `NoData` (radio cut / inter-SID gap) continues it *"at a regular low rate"* (§4), speech ends it — sharing **one** `DecoderState` across speech decode and §6.1 synthesis for click-free §4 cross-transition continuity. Frame *classification* is left to the caller (`RxFrame::Speech/Sid/NoData`), as `NoiseEvaluator` leaves VAD to the caller. Closes the §5.1 → §5.2 → §6.1 loop and the §6 dispatch. The GSM 06.32 VAD, the §5.2 SID-frame layout (95-bit class-I SID code word per GSM 05.03 table 2), the receive-side SID detector, and the GSM 06.31 TX DTX handler are all now implemented — see the VAD and DTX rows. |
| VAD (GSM 06.32) | Complete clause-3 fixed-point algorithm — **conformance-exact**: all 20 official EN 300 965 test cases reproduce every `*.VAD` reference flag bit-exactly in CI | The `vad` module implements adaptive filtering + pseudo-float energy computation, 4-frame ACF averaging, predictor values (Schur → step-up → autocorrelated predictors), spectral comparison, periodicity detection/updating, threshold adaptation (Table 3.2 pseudo-float constants), VAD decision, hangover, and the clause 3.10 information-tone detector (Hanning window, 4th-order analysis, pole-frequency + prediction-gain tests; downlink/uplink modes). The encoder surfaces the clause-3 tap (`L_ACF`, `scalauto`, LTP lags, `sof[]`) via `EncoderState::encode_frame_with_vad_tap` → `VadTap`. |
| DTX (GSM 06.31 + 06.12 SID) | Complete transmit side — **conformance-exact**: encoder + VAD + TX DTX handler + SID encoding reproduce all 20 EN 300 965 corpus `*.COD` streams bit-exactly (SP flags, SID frames, scheduling) in CI | The `dtx` module: GSM 06.12 §5.2 SID frames (mean LARs + mean `xmax` ×4 + the 95-bit all-zero SID code word in the class-I `Xmc` positions per GSM 05.03 table 2, all remaining bits zero), the §5.1 averaging (window = the four VAD=0 frames preceding the replaced frame; `floor(x+1/2)` means — both corpus-pinned), the GSM 06.31 §5.1.1 `TxDtxHandler` (after-reset + normal hangover, `<24`-elapsed short-burst SID repetition, continuous per-frame SID updates), the §6.1.1 ternary SID detector (`sid_flag`, deviation thresholds <2/<16), and the §6.1 table-1 RX classification (`classify_rx_frame`: valid/invalid SID, lost speech/SID, unusable-ignore). The GSM 06.11 (EN 300 962) substitution-and-muting legs are implemented in `RxDtxHandler`: first-lost-speech-frame repetition, the clause 6 muting (`Xmaxcr` −4 per frame to silence within 320 ms, random grids) down to the table-1 silence frame, last-valid-SID substitution for a single lost SID, and comfort-noise muting from the second lost SID onward (`tests/lost_frame_06_11.rs`). |
| Half rate (GSM 06.20) | **Complete codec (functional): decoder + full clause 4.1 encoder** — homing legs **sample-exact** both ways against the staged GSM 06.07 corpus; decoder waveform agreement vs `SEQ01..04.OUT` pinned in CI (mean per-frame correlation 0.99; our own encode→decode tracks the input at 0.55, the reference decoder at 0.52–0.68); encoder per-parameter agreement vs `SEQ01..03.COD` pinned in CI end to end (INT_LPC 91.5%, MODE 91.3%, unvoiced VSELP codes 77–81%, unvoiced GSP0 68.5%, lag levels ±1 65.6%, voiced codes 20.3%) and stage-isolated on the reference's upstream parameters (MODE 92.4%, unvoiced codes 88–91%, voiced codes 41.1%, GSP0 82.2% / 68.7%); bit-exactness is out of reach without the deliberately-unstaged GSM 06.06 fixed-point ANSI-C (clean-room posture) — the residue is ±1/6-sample lag choices and near-tie codebook decisions | The `hr` module family: annex A table A.1 parameter set (`HrParameters`, MODE-dependent subframe split), annex B `b1..b112` bit packing (tables B.1/B.2), the GSM 06.07 conformance word layouts, `hr::tables` — every ROM table of the codec transcribed from the staged data extracts — `hr::decode` (`HrDecoder`): LPC dequantisation through the three-segment reflection-coefficient VQ, R0 energy decode, soft interpolation with the clause 4.1.6 stability fallback, VSELP + adaptive-codebook excitation with 1/6-sample fractional lags (eq. (100) periodic extension), GSP0 gain reconstruction, adaptive pitch prefilter, synthesis filter, adaptive spectral postfilter (the eqs. (158)–(164) SST-smoothed numerator over A(z·0,75) + brightness + AGC) and decoder homing — and `hr::encode` (`HrAnalyzer`, `HrEncoder`): the clause 4.1.2 look-ahead segmentation (35 samples), input high-pass, FLAT covariance lattice, the three-segment AFLAT VQ search, R0 coding, the INT_LPC decision, the clause 4.1.7 spectral noise weighting filter, the clause 4.1.8 open-loop lag search / frame lag trajectory / voicing mode and closed-loop lag search, clause 4.1.9 harmonic noise weighting, the clause 4.1.10 VSELP code searches (two 7-bit codebooks in MODE 0, one 9-bit codebook in MODEs 1–3, with decorrelation), the clause 4.1.11 multimode `{P0,GS}` gain quantisation and clause 5.3 encoder homing; `make_hr_encoder` / `make_hr_decoder` and the `gsm-hr` registry entry carry annex-B frames as 14-byte packets. EN 300 969 prints the codec as floating-point functional equations (clause 5.1 places the bit-exact arithmetic in GSM 06.06), so both directions are implemented in double precision over the staged ROM data; the readings the corpus pinned (lattice sign convention, Q14 gain components with `Rmax = 4096²` on the unity-gain coded signal, the halved printed high-pass coefficients, the eq. (67) threshold read in dB, the ceiling convention of the correlation interpolator) are documented in `hr::decode`, `hr::encode` and `hr::lag`. |
...
* **Reference-sequence formats** — [`SyncFormats`] records the §6.3.3.4
  ...
  are staged in-repo (`tests/fixtures/etsi-fr/`):
  `tests/conformance_etsi_fr.rs` verifies the `BITSYNC.INP` 13-trial
  sweep detects exactly the genuine homing-frame triplet, and that
  [`FrameSyncTable`] built from the genuine `SEQSYNC.INP` special frame
  reproduces `SYNC000.COD..SYNC149.COD` bit-exactly with all 160
  outputs pairwise distinct (the staged `SYNC15x` files byte-duplicate
  `SYNC05x` — a corpus defect pinned as a tripwire).
...
overflow / critical-path sequences of §6.3.1 / §6.3.2) is now staged
in-repo and run bit-exactly by `tests/conformance_etsi_fr.rs` (see the
Status table). Independently of the corpus, the **invariants**
...
consumes — closing the §5.1 → §6.1 loop end-to-end. The §5.1 means
round as `floor(x + 1/2)` — the fixed-point `(sum + N/2) >> log2(N)`.
The spec prose leaves the rounding unstated, but the EN 300 965
conformance corpus pins it from both sides (a negative-tie LAR mean in
PRED1 and positive .75/.8 means in BAD_SP/ADAPT_*), and every
reference SID frame is reproduced bit-exactly with this rule
(`tests/conformance_vad.rs`).

### DTX system status

The pieces formerly docs-blocked are now staged and implemented: the
**VAD flag** (GSM 06.32, `vad` module — see the VAD row), the §5.2
SID-frame layout with the 95-bit all-zero SID code word in the class-I
`Xmc` positions (GSM 06.12 + GSM 05.03 table 2, `dtx` module), the
receive-side SID detector (`sid_flag`, the GSM 06.31 §6.1.1 ternary
flag with its <2 / <16 deviation thresholds), and the GSM 06.31 §5.1.1
TX DTX handler (`TxDtxHandler`: hangover, short-burst SID repetition,
continuous SID updating). The complete transmit side — encoder + VAD +
DTX + SID encoding — reproduces all 20 EN 300 965 corpus `*.COD`
streams bit-exactly, SID frames and SP flags included
(`tests/conformance_vad.rs`).
## Robustness & round-trip quality gates

Five libFuzzer targets (`fuzz/`, run bounded on every push by
`.github/workflows/fuzz.yml`) harden the hostile-input surfaces:
full-rate decode under all three frame packings with homing live,
half-rate frame decode totality over the whole annex-A parameter
space, packing round-trip walkers (in-band / `.gsm` / MS-GSM /
conformance words), fuzz-driven RX-DTX + GSM 06.11 loss-burst
streams, and the encode→pack→parse→decode round trip.
...
sample-for-sample (decode). Alongside the official ETSI corpus (now
staged and CI-enforced, see above), the crate is validated against
independent implementations invoked as opaque black boxes:
...
* `docs/audio/gsm/etsi-en-300969-gsm-06.20-half-rate.pdf` — ETSI
  ...
  transcoding* (VSELP), with its ROM data staged as extracts under
  `docs/audio/gsm/halfrate-tables/` and the GSM 06.07 conformance
  corpus under `docs/audio/gsm/conformance/halfrate-06.07/`. Both
  directions are implemented by the `hr` module family (see the
  status table).
